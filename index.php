...
$ch = curl_init($base_url.'/v1.2.0-beta/tokenized/checkout/token/grant');
...
$ch = curl_init($base_url.'/v1.2.0-beta/tokenized/checkout/create');
curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
...
$ch = curl_init($base_url.'/v1.2.0-beta/tokenized/checkout/execute');
curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($post_token));
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HTTPHEADER, [
'Content-Type:application/json',
'Content-Length:'.strlen(json_encode($post_token)),
'Authorization:'.$auth,
'X-APP-Key:'.$app_key
]);
